...
def add_mbf_abstract_nav_params(gen):
    ...
    gen.add("inter_frequency", double_t, 0,
            "The rate in Hz at which to run the planning loop", 0, 0, 100)
    gen.add("inter_patience", double_t, 0,
            "How long the inter will wait in seconds in an attempt to find a valid plan before giving up", 5.0, 0, 100)
    gen.add("inter_max_retries", int_t, 0,
            "How many times we will recall the inter in an attempt to find a valid plan before giving up", -1, -1, 1000)
